...
@keras_export('keras.layers.OGRU', v1=[])
class OGRU(recurrent.DropoutRNNCellMixin, recurrent.GRU):
  """Gated Recurrent Unit - Cho et al. 2014.
    ...
    if _use_new_code():
      ...
      if tf.executing_eagerly():
        ...
        if can_use_gpu:
          ...
          last_output, outputs, new_h, runtime = standard_ogru(
              **normal_gru_kwargs)
      else:
        last_output, outputs, new_h, runtime = gru_with_backend_selection(
            **normal_gru_kwargs)
...
def standard_ogru(inputs, init_h, kernel, recurrent_kernel, bias, mask,
                 time_major, go_backwards, sequence_lengths,
                 zero_output_for_mask):
  """GRU with standard kernel implementation.
  ...
  def step(cell_inputs, cell_states):
    ...
    recurrent_z, recurrent_r, recurrent_h = tf.split(matrix_inner, 3,
                                                            axis=1)
    r = tf.sigmoid(x_r + recurrent_r)
    z = tf.sigmoid(tf.multiply(x_z, r)  + recurrent_z)
